...
import { SHOP_ITEMS, WEAPON_DEFINITIONS, DRUG_DEFINITIONS, ALCOHOL_DEFINITIONS, FAKE_ID_DEFINITIONS, STOLEN_GOODS_DEFINITIONS, ILLEGAL_SEED_DEFINITIONS, HIGH_RISK_LOCATIONS } from '@/lib/game/gameData';
...
export default function Shop() {
  ...
  function purchase() {
    ...
    const bmAlcohol   = cart.filter(c => ALCOHOL_DEFINITIONS.some(a => a.id === c.itemId));
    const bmFakeIds   = cart.filter(c => FAKE_ID_DEFINITIONS.some(f => f.id === c.itemId));
    const bmStolen    = cart.filter(c => STOLEN_GOODS_DEFINITIONS.some(g => g.id === c.itemId));
    const bmSeeds     = cart.filter(c => ILLEGAL_SEED_DEFINITIONS.some(sd => sd.id === c.itemId));
    const bmAll = [...bmWeapons, ...bmDrugs, ...bmAlcohol, ...bmFakeIds, ...bmStolen, ...bmSeeds];
    const normalItems = cart.filter(c => !bmAll.includes(c));
    ...
    bmAlcohol.forEach(a => { for (let i = 0; i < a.quantity; i++) dispatch({ type: 'BUY_ALCOHOL', payload: a.itemId }); });
    bmFakeIds.forEach(f => { for (let i = 0; i < f.quantity; i++) dispatch({ type: 'BUY_FAKE_ID', payload: f.itemId }); });
    bmStolen.forEach(g => { for (let i = 0; i < g.quantity; i++) dispatch({ type: 'BUY_STOLEN_GOODS', payload: g.itemId }); });
    bmSeeds.forEach(sd => { for (let i = 0; i < sd.quantity; i++) dispatch({ type: 'BUY_ILLEGAL_SEED', payload: sd.itemId }); });
  ...
  function riskColor(risk: number): string {
    if (risk >= 35) return C.red;
    if (risk >= 18) return C.orange;
    return C.green;
  }

  function BlackMarketRow({ item, category }: { item: { id: string; name: string; price: number; description: string; riskPercent: number; demand: string; supply: string }; category: string }) {
    const cartItem = cart.find(c => c.itemId === item.id);
    const qty = cartItem?.quantity ?? 0;
    return (
      <View style={{ padding: 16, marginBottom: 8, backgroundColor: C.surface, borderWidth: 1, borderColor: qty > 0 ? C.gold : C.border, borderRadius: 10 }}>
        <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
          <View style={{ flex: 1 }}>
            <Text style={{ color: C.textPrimary, fontWeight: '800', fontSize: 14 }}>{item.name}</Text>
            <Text style={{ color: C.textSub, fontSize: 12, marginTop: 4 }}>{item.description}</Text>
          </View>
          <Text style={{ color: C.gold, fontWeight: '800', fontSize: 15, marginLeft: 12 }}>{formatMoney(item.price)}</Text>
        </View>
        <View style={{ flexDirection: 'row', gap: 12, marginTop: 10 }}>
          <Text style={{ color: C.textMuted, fontSize: 11 }}>Demand: <Text style={{ color: C.textSub, fontWeight: '700' }}>{item.demand}</Text></Text>
          <Text style={{ color: C.textMuted, fontSize: 11 }}>Supply: <Text style={{ color: C.textSub, fontWeight: '700' }}>{item.supply}</Text></Text>
          <Text style={{ color: C.textMuted, fontSize: 11 }}>Risk: <Text style={{ color: riskColor(item.riskPercent), fontWeight: '700' }}>{item.riskPercent}%</Text></Text>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 8, marginTop: 10 }}>
          {qty > 0 && <Pressable onPress={() => removeFromCart(item.id)} style={{ width: 32, height: 32, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: C.gold, borderRadius: 6 }}><Text style={{ color: C.gold, fontWeight: '800', fontSize: 20, marginTop: -2 }}>−</Text></Pressable>}
          {qty > 0 && <Text style={{ color: C.textPrimary, fontWeight: '800', width: 24, textAlign: 'center' }}>{qty}</Text>}
          <Pressable onPress={() => addToCart(item.id, item.price, category)} style={{ width: 32, height: 32, alignItems: 'center', justifyContent: 'center', backgroundColor: '#3A2800', borderWidth: 1, borderColor: C.gold, borderRadius: 6 }}><Text style={{ color: C.gold, fontWeight: '800', fontSize: 20, marginTop: -2 }}>+</Text></Pressable>
        </View>
      </View>
    );
  }
            ...
            <>
              {!isHighRisk ? (
                ...
                <>
                  ...
                  <Text style={{ color: C.textMuted, fontSize: 11, fontWeight: '800', letterSpacing: 1.5, marginBottom: 8, marginTop: 8 }}>💊 DRUGS</Text>
                  {DRUG_DEFINITIONS.map(d => (
                    <BlackMarketRow key={d.id} item={d} category="blackmarket" />
                  ))}

                  <Text style={{ color: C.textMuted, fontSize: 11, fontWeight: '800', letterSpacing: 1.5, marginBottom: 8, marginTop: 8 }}>🍺 ALCOHOL (UNLICENSED)</Text>
                  {ALCOHOL_DEFINITIONS.map(a => (
                    <BlackMarketRow key={a.id} item={a} category="blackmarket" />
                  ))}

                  <Text style={{ color: C.textMuted, fontSize: 11, fontWeight: '800', letterSpacing: 1.5, marginBottom: 8, marginTop: 8 }}>🪪 FAKE IDS</Text>
                  {FAKE_ID_DEFINITIONS.map(f => (
                    <BlackMarketRow key={f.id} item={f} category="blackmarket" />
                  ))}

                  <Text style={{ color: C.textMuted, fontSize: 11, fontWeight: '800', letterSpacing: 1.5, marginBottom: 8, marginTop: 8 }}>📦 STOLEN GOODS</Text>
                  {STOLEN_GOODS_DEFINITIONS.map(g => (
                    <BlackMarketRow key={g.id} item={g} category="blackmarket" />
                  ))}

                  <Text style={{ color: C.textMuted, fontSize: 11, fontWeight: '800', letterSpacing: 1.5, marginBottom: 8, marginTop: 8 }}>🌱 ILLEGAL SEEDS</Text>
                  {ILLEGAL_SEED_DEFINITIONS.map(sd => (
                    <BlackMarketRow key={sd.id} item={sd} category="blackmarket" />
                  ))}
                </>
              )}
            </>
      ...
      {cart.length > 0 && (
        <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, backgroundColor: C.bg, borderTopWidth: 1, borderTopColor: C.border, shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.5, shadowRadius: 8, elevation: 10 }}>
          ...
          <ScrollView style={{ maxHeight: 100 }}>
            {cart.map(c => {
              const item = [...allItems, ...WEAPON_DEFINITIONS, ...DRUG_DEFINITIONS, ...ALCOHOL_DEFINITIONS, ...FAKE_ID_DEFINITIONS, ...STOLEN_GOODS_DEFINITIONS, ...ILLEGAL_SEED_DEFINITIONS].find(i => i.id === c.itemId);
              return (
